...
Sword::Sword(const short type, unsigned level, unsigned damage_min, unsigned damage_max, unsigned range,
	unsigned value, std::string texture_file)
	: MeleeWeapon(type, level, damage_min, damage_max, range, value, texture_file)
{
	// Visual Weapon
...
void Sword::update(const sf::Vector2f& mouse_pos_view, const sf::Vector2f center)
{
	...
	if (this->attackTimer.getElapsedTime().asMilliseconds() < this->attackTimerMax / 2)
	{
		...
		this->weaponSprite.setPosition(center.x + normVec.x * 10.f, center.y + normVec.y * 10.f);
	}
}
